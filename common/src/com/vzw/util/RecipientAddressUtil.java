...
public class RecipientAddressUtil {
	private static final Logger					logger = Logger.getLogger(RecipientAddressUtil.class);
	...
	private static final Pattern				LONG_CODE_REGEX_PATTERN = Pattern.compile("([0-9]{3,6}|[0-9]{8,9}|[0-9]{12,25})");
	...
	public RecipientAddress parseEmail(String input) {
		...
		if (!org.apache.commons.lang3.StringUtils.isBlank(input)) {
			...
			if (m.matches()) {
				...
				res.setNormalized(_input);
				res.setFormatted(_input);
				LogUtil.debug(logger, "Failed to parse email: {0}", input);
			}
			...
			res.setNormalized("");
			res.setFormatted("");
			LogUtil.debug(logger, "Failed to parse mdn (empty): {0}", input);
		}
			...
			if (m.matches()) {
				...
				if (m.matches()) {
					...
					if (s1.length() == 10) {
						...
						res.setNormalized(_input2);
						LogUtil.error(logger, "Failed to parse mdn as north American number (not possible to reach here): {0}", input);
					}
					...
					if (m.matches()) {
						...
						res.setNormalized(_input1);
						res.setFormatted(_input1);
						LogUtil.debug(logger, "Failed to parse mdn: {0}", input);
					}
				}
			}
		}
