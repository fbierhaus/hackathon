...
public class SmsReceiver extends BroadcastReceiver {
    ...
    protected void onReceiveWithPrivilege(Context context, Intent intent, boolean privileged) {
    	if (MmsConfig.isTabletDevice()) {
    		// treating it as tablet so dont handle SMS
    		if (Logger.IS_ACRA_ENABLED) {
    			Logger.debug(getClass(), "got incoming SMS on tablet - dont handle it");
    		}
    		return;
    	}
        ...
        if (!privileged && Intents.SMS_RECEIVED_ACTION.equals(action)) {
            return;
        }
        ...
        // Abort (http://50.17.243.155/bugzilla/show_bug.cgi?id=9)
        // changed it so we only abort broadcast if SMS_RECEIVED_ACTION - not otherwise
        if (isOrderedBroadcast() && Intents.SMS_RECEIVED_ACTION.equals(action)) {
            if (Logger.IS_WARNING_ENABLED) {
                Logger.debug(getClass(), "onReceiveWithPrivilege - abort broadcast");
            }
        	abortBroadcast();
        }
    }
    ...
    public static void beginStartingService(Context context, Intent intent) {
        synchronized (mStartingServiceSync) {
            ...
            if (Logger.IS_WARNING_ENABLED) {
                Logger.debug(SmsReceiver.class, "onReceiveWithPrivilege - start service");
            }
