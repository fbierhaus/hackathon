...
public class RateController {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		if (Logger.IS_DEBUG_ENABLED) {
            	Logger.debug(getClass(), "Intent received: " + intent);
            }
    ...
    public static void init(Context context) {
        		if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(RateController.class, "RateController.init()");
        }

        if (sInstance != null) {
        	Logger.warn(RateController.class, "Already initialized.");
        }
    ...
    public final boolean isLimitSurpassed() {
        ...
        Cursor c = SqliteWrapper.query(mContext, mContext.getContentResolver(),
        		VZUris.getMmsRateUri(), new String[] { "COUNT(*) AS rate" },
    ...
    synchronized private int waitForAnswer() {
        for (int t = 0; (mAnswer == NO_ANSWER) && (t < ANSWER_TIMEOUT); t += 1000) {
            try {
    		if (Logger.IS_DEBUG_ENABLED) {
                	Logger.debug(getClass(), "Waiting for answer..." + t / 1000);
                }
                wait(1000L);
            } catch (InterruptedException _) {
                 // Ignore it.
