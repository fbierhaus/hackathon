...
public class SmilPlayer implements Runnable {
    ...
    public synchronized void play() {
        if (!isPlayingState()) {
            ...
            Logger.error(getClass(), "Error State: Playback is playing!");
        }
    }
    ...
    public synchronized void pause() {
        if (isPlayingState()) {
        	...
        	Logger.error(getClass(), "Error State: Playback is not playing!");
        }
    }
    ...
    public synchronized void start() {
        if (isPausedState()) {
        	...
        	Logger.error(getClass(), "Error State: Playback can not be started!");
        }
    }
        ...
        for (int i = mActiveElements.size() - 1; i >= 0; i--) {
            ...
            if ((offset >= 0) && (offset <= mCurrentTime)) {
                    if (Logger.IS_DEBUG_ENABLED) {
                        Logger.debug(getClass(), "[SEEK]  " + " at " + mCurrentTime + " " + element);
                    }
    ...
    private synchronized void waitForEntry(long interval)
            ...
            if (Logger.IS_DEBUG_ENABLED) {
                Logger.debug(getClass(), "Waiting for " + interval + "ms.");
            }
    ...
    private synchronized void endActiveElements() {
        for (int i = mActiveElements.size() - 1; i >= 0; i--) {
            ElementTime element = mActiveElements.get(i);
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[STOP]  " + " at " + mCurrentTime + " " + element);
                }
            element.endElement();
        }
    }
    ...
    private synchronized void pauseActiveElements() {
        for (int i = mActiveElements.size() - 1; i >= 0; i--) {
            ElementTime element = mActiveElements.get(i);
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[PAUSE]  " + " at " + mCurrentTime + " " + element);
                }
    ...
    private synchronized void resumeActiveElements() {
        ...
        for (int i = 0; i < size; i++) {
            ElementTime element = mActiveElements.get(i);
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[RESUME]  " + " at " + mCurrentTime + " " + element);
                }
    ...
    private synchronized void waitForWakeUp() {
        try {
        	...
        	Logger.error(getClass(), "Unexpected InterruptedException.", e);
        }
    }
    ...
    private synchronized void actionEntry(TimelineEntry entry) {
        switch (entry.getAction()) {
            case TimelineEntry.ACTION_BEGIN:
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[START] " + " at " + mCurrentTime + " " + entry.getElement());
                }
            ...
            case TimelineEntry.ACTION_END:
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[STOP]  " + " at " + mCurrentTime + " " + entry.getElement());
                }
    ...
    private void dumpAllEntries() {
            for (TimelineEntry entry : mAllEntries) {
                if (Logger.IS_DEBUG_ENABLED) {
                    Logger.debug(getClass(), "[Entry] " + entry);
                }
            }
    }
}
