...
public class LayoutModel extends Model {
        ...
        for (RegionModel r : regions) {
            ...
            if (rId.equals(IMAGE_REGION_ID)) {
                ...
                if (Logger.IS_DEBUG_ENABLED) {
                	Logger.debug(getClass(), "Found non-standard region: " + rId);
                }
    ...
    public RegionModel findRegionById(String rId) {
        if (IMAGE_REGION_ID.equals(rId)) {
            ...
            if (Logger.IS_DEBUG_ENABLED) {
            	Logger.debug(getClass(), "Region not found: " + rId);
            }
            return null;
        }
    }
    ...
    public void changeTo(int layout) {
        ...
        if (mLayoutType != layout) {
            switch (layout) {
                ...
                break;
                default: {
                	Logger.warn(getClass(), "Unknown layout type: " + layout);
                }
            }
        } else {
            if (Logger.IS_DEBUG_ENABLED) {
            	Logger.debug(getClass(), "Skip changing layout.");
            }
        }
    }
