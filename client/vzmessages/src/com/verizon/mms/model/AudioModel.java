...
public class AudioModel extends MediaModel {
            ...
            try {
                if (c.moveToFirst()) {
                    ...
                    if (isFromMms) {
                        ...
                        if (!TextUtils.isEmpty(album)) {
                            mExtras.put("album", album);
                           	mMemorySize = album.length() * 2;
                        }

                        String artist = c.getString(c.getColumnIndexOrThrow("artist"));
                        if (!TextUtils.isEmpty(artist)) {
                            mExtras.put("artist", artist);
                           	mMemorySize += artist.length() * 2;
                        }
                        
                        int durCol = c.getColumnIndex(AudioColumns.DURATION);
                        if (durCol != -1) {
                    ...
                    if (Logger.IS_DEBUG_ENABLED) {
                        Logger.debug(getClass(), "New AudioModel created:" + " mSrc=" + mSrc
                                + " mContentType=" + mContentType + " mUri=" + uri + " mExtras=" + mExtras);
                    }
                } else {
    ...
    public void handleEvent(Event evt) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "Handling event: " + evtType + " on " + this);
        }
