...
public class VideoModel extends RegionMediaModel {
            ...
            try {
                if (c.moveToFirst()) {
                    ...
                    if (ImageModel.isMmsUri(uri)) {
                        ...
                        int durCol = c.getColumnIndex(VideoColumns.DURATION);
                        
                        if (durCol != -1) {
                            mDuration = c.getInt(durCol);
                        }
                    }
                    ...
                    if (Logger.IS_DEBUG_ENABLED) {
                    	Logger.debug(getClass(), "New VideoModel created:"
                                + " mSrc=" + mSrc
    ...
    public void handleEvent(Event evt) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "[VideoModel] handleEvent " + evt.getType() + " on " + this);
        }
    ...
    private Bitmap internalGetBitmap(Uri uri) {
        ...
        if (bm == null) {
            try {
            	bitmapLoaded = true;
                bm = bitmapMgr.createVideoThumbnail(mContext, uri);

                if (bm != null) {
                    mBitmap = bm;
        			mMemorySize = BitmapManager.getBitmapSize(bm);
                }
            } catch (OutOfMemoryError ex) {
                // fall through and return a null bitmap. The callers can handle a null
                // result and show R.drawable.ic_missing_thumbnail_video
            }
        }
        return bm;
    }
