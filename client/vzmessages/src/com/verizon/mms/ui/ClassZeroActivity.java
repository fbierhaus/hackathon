 ...
 * it.
 */
public class ClassZeroActivity extends VZMActivity {
    ...
    private AppAlignedDialog mDialog = null;
    ...
    private void saveMessage() {
        ...
        if (!mRead && messageUri != null) {
            MessagingNotification.nonBlockingUpdateNewMessageIndicator(this, true, false, null);
        }
    }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mDialog = new AppAlignedDialog(this,
				R.drawable.dialog_alert, 
				R.string.app_name, 
				messageChars.toString());
        mDialog.setCancelable(false);
		Button saveButton = (Button) mDialog.findViewById(R.id.positive_button);
		...
		Button cancelButton = (Button) mDialog.findViewById(R.id.negative_button);
    ...
    protected void onStart() {
        ...
        if (mTimerSet <= now) {
            ...
            if (Logger.IS_DEBUG_ENABLED) {
            	Logger.debug(getClass(), "onRestart time = " + Long.toString(mTimerSet) + " "
                        + this.toString());
            }
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "onSaveInstanceState time = " + Long.toString(mTimerSet)
                    + " " + this.toString());
        }
    }
    ...
    protected void onStop() {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "onStop time = " + Long.toString(mTimerSet)
                    + " " + this.toString());
        }
    }
    ...
    private Uri replaceMessage(SmsMessage sms) {
        ...
        Cursor cursor = SqliteWrapper.query(this, resolver, VZUris.getSmsInboxUri(),
    ...
    private Uri storeMessage(SmsMessage sms) {
        ...
        ContentResolver resolver = getContentResolver();
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "storeMessage " + this.toString());
        }
        return SqliteWrapper.insert(this, resolver, VZUris.getSmsInboxUri(), values);
    }
}
