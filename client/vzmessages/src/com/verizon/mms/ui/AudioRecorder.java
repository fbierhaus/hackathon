...
public class AudioRecorder extends Activity 
    ...
    public void onCreate(Bundle icycle) {
        ...
        if (i != null) {
            ...
            if (bund != null) {
                mMaxFileSize = bund.getInt(EXTRA_MAX_BYTES, -1);
            }
        }
    ...
    private int getPlaylistId(Resources res) {
        ...
        if (cursor == null) {
            if (Logger.IS_DEBUG_ENABLED) {
                Logger.debug(getClass(), "query returns null");
            }
        }
        int id = -1;
        if (cursor != null) {
    ...
    private Uri createPlaylist(Resources res, ContentResolver resolver) {
        ...
        if (uri == null) {
            final AppAlignedDialog d = new AppAlignedDialog(this,
    				...
    				R.string.app_name, 
    				R.string.error_mediadb_new_record);
            d.setCancelable(false);
    		Button saveButton = (Button) d.findViewById(R.id.positive_button);
    		saveButton.setText(R.string.button_ok);
    ...
    private Uri addToMediaDB(File file) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(getClass(), "Inserting audio record: " + cv.toString());
        }
        ContentResolver resolver = getContentResolver();
        Uri base = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(getClass(), "ContentURI: " + base);
        }
