...
public class VCardComposer {
    ...
    static final private HashSet<String> addrTypes = new HashSet<String>(
            Arrays.asList("WORK", "HOME"));
    ...
    private static final HashMap<Integer, String> contactMethodTypeMap = new HashMap<Integer, String>();

    static {
        phoneTypeMap.put(VCardContacts.Phones.TYPE_HOME, "HOME");
        phoneTypeMap.put(VCardContacts.Phones.TYPE_MOBILE, "CELL");
        phoneTypeMap.put(VCardContacts.Phones.TYPE_WORK, "WORK");
        // FAX_WORK not exist in vcard spec. The approximate is the combine of
        // WORK and FAX, here only map to FAX
        phoneTypeMap.put(VCardContacts.Phones.TYPE_FAX_WORK, "WORK;FAX");
        phoneTypeMap.put(VCardContacts.Phones.TYPE_FAX_HOME, "HOME;FAX");
        phoneTypeMap.put(VCardContacts.Phones.TYPE_PAGER, "PAGER");
        phoneTypeMap.put(VCardContacts.Phones.TYPE_OTHER, "X-OTHER");
        contactMethodTypeMap.put(VCardContacts.ContactMethods.TYPE_HOME, "HOME");
        contactMethodTypeMap.put(VCardContacts.ContactMethods.TYPE_WORK, "WORK");
    ...
    public String createVCard(ContactStruct struct, int vcardversion)
        ...
        String value = struct.getGeoCoord();
        if (!isNull(value)) {
            appendExtn("GEO", value);
        }
        
        value = struct.getURL();
        if (!isNull(value)) {
            appendExtn("URL", value);
        }

        if (!isNull(struct.getCompany())) {
            mResult.append("ORG:").append(struct.getCompany()).append(mNewline);
        }

        if (struct.getNotes().size() > 0 && !isNull(struct.getNotes().get(0))) {
        ...
        //ishaque removed adding photoBytes as it is not required presently
        ...
        if (struct.getContactMethodsList() != null) {
            appendContactMethodStr(struct.getContactMethodsList(), vcardversion);
        }
        
        value = struct.getVZWLocation();
        if (!isNull(value)) {
            appendExtn(ContactStruct.VZW_LOCATION, value);
        }
    ...
    private void appendExtn(String key, String value) {
        mResult.append(key + ":" + value + mNewline);
    }
    /**
    ...
    private void appendNameStr(String name) {
        mResult.append("N:;").append(name.replace(" ", ";")).append(";;;" + mNewline);
    ...
    private String getPhoneTypeStr(PhoneData phone) {
        ...
        } else if (phoneType == VCardContacts.Phones.TYPE_CUSTOM) {
    ...
    private void appendContactMethodStr(
        ...
        for (ContactStruct.ContactMethod contactMethod : contactMList) {
            ...
            case VCardContacts.KIND_EMAIL:
                String mailType = "INTERNET";
                if (!isNull(contactMethod.data)) {
                    int methodType = new Integer(contactMethod.type).intValue();
                    if (contactMethodTypeMap.containsKey(methodType)) {
                        mailType = contactMethodTypeMap.get(methodType);
                ...
                break;
            case VCardContacts.KIND_POSTAL:
                String addrType = "WORK";
                if (!isNull(contactMethod.data)) {
                    int methodType = new Integer(contactMethod.type).intValue();
                    
                    if (contactMethodTypeMap.containsKey(methodType)) {
                        addrType = contactMethodTypeMap.get(methodType);
                    } else if (!isNull(contactMethod.label) && addrTypes.contains(contactMethod.label
                            .toUpperCase())) {
                       addrType = contactMethod.label.toUpperCase();
                    }
                    
                    if (addrMap.containsKey(contactMethod.data)) {
                        addrType = addrMap.get(contactMethod.data) + joinMark 
                                + addrType;
                    }
                    addrMap.put(contactMethod.data, addrType);
                }
                break;
            default:
                break;
        ...
        String adrField;
        String labelField;
        for (Map.Entry<String, String> addr : addrMap.entrySet()) {
            if (version == VERSION_VCARD21_INT) {
                adrField = "ADR;";
                labelField = "LABEL;";
            } else {
            ...
            mResult.append(adrField).append(addr.getValue()).append(":;;").append(addr.getKey()
                    .replace(ContactStruct.ADDRESS_SEPERATOR, ";")).append(mNewline);
            
            //add the label field too
            //custom field to match the old vzm app
            mResult.append(labelField).append(addr.getValue()).append(":;").append(addr.getKey())
                    .append(mNewline);
        }
    }
}
