...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String SMS_UPDATE_THREAD_READ_BODY = "  UPDATE threads SET read = "
            + "    CASE (SELECT COUNT(*)" + "          FROM sms" + "          WHERE " + Sms.READ + " = 0"
            + "            AND " + Sms.THREAD_ID + " = threads._id)" + "      WHEN 0 THEN 1" + "      ELSE 0"
            + "    END" + "  WHERE threads._id = new." + Sms.THREAD_ID + "; ";

    private static final String UPDATE_THREAD_COUNT_ON_NEW = "  UPDATE threads SET message_count = "
            + "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads " + "      ON threads._id = "
            + Sms.THREAD_ID + "      WHERE " + Sms.THREAD_ID + " = new.thread_id" + "        AND sms."
            + Sms.TYPE + " != 3) + " + "     (SELECT COUNT(pdu._id) FROM pdu LEFT JOIN threads "
            + "      ON threads._id = " + Mms.THREAD_ID + "      WHERE " + Mms.THREAD_ID + " = new.thread_id"
            + "        AND (m_type=132 OR m_type=130 OR m_type=128)" + "        AND " + Mms.MESSAGE_BOX
            + " != 3) " + "  WHERE threads._id = new.thread_id; ";

    private static final String UPDATE_THREAD_COUNT_ON_OLD = "  UPDATE threads SET message_count = "
            + "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads " + "      ON threads._id = "
            + Sms.THREAD_ID + "      WHERE " + Sms.THREAD_ID + " = old.thread_id" + "        AND sms."
            + Sms.TYPE + " != 3) + " + "     (SELECT COUNT(pdu._id) FROM pdu LEFT JOIN threads "
            + "      ON threads._id = " + Mms.THREAD_ID + "      WHERE " + Mms.THREAD_ID + " = old.thread_id"
            + "        AND (m_type=132 OR m_type=130 OR m_type=128)" + "        AND " + Mms.MESSAGE_BOX
            + " != 3) " + "  WHERE threads._id = old.thread_id; ";

    private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE = "BEGIN"
            + "  UPDATE threads SET" + "    date = (strftime('%s','now') * 1000), " + "    snippet = new."
            + Sms.BODY + ", " + "    snippet_cs = 0" + "  WHERE threads._id = new." + Sms.THREAD_ID + "; "
    ...
    private static final String PDU_UPDATE_THREAD_READ_BODY = "  UPDATE threads SET read = "
            + "    CASE (SELECT COUNT(*)" + "          FROM " + MmsProvider.TABLE_PDU + "          WHERE "
            + Mms.READ + " = 0" + "            AND " + Mms.THREAD_ID + " = threads._id "
            + "            AND (m_type=132 OR m_type=130 OR m_type=128)) " + "      WHEN 0 THEN 1"
            + "      ELSE 0" + "    END" + "  WHERE threads._id = new." + Mms.THREAD_ID + "; ";

    private static final String PDU_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE = "BEGIN"
            + "  UPDATE threads SET" + "    date = (strftime('%s','now') * 1000), " + "    snippet = new."
            + Mms.SUBJECT + ", " + "    snippet_cs = new." + Mms.SUBJECT_CHARSET
            + "  WHERE threads._id = new." + Mms.THREAD_ID + "; " + UPDATE_THREAD_COUNT_ON_NEW
    ...
    private static final String PART_UPDATE_THREADS_ON_DELETE_TRIGGER = "CREATE TRIGGER update_threads_on_delete_part "
            ...
            + "     AND part.mid = pdu._id)" + "   WHEN 0 THEN 0 " + "   ELSE 1 " + "   END; " + " END";
    ...
    public static void updateThread(SQLiteDatabase db, long thread_id) {
        ...
        int rows = db.delete("threads", "_id = ? AND _id NOT IN" + "          (SELECT thread_id FROM sms "
                + "           UNION SELECT thread_id FROM pdu)", new String[] { String.valueOf(thread_id) });
        if (rows > 0) {
        ...
        db.execSQL("  UPDATE threads SET message_count = "
                ...
                + "        AND "
                + Mms.MESSAGE_BOX + " != 3) " + "  WHERE threads._id = " + thread_id + ";");

        // Update the date and the snippet (and its character set) in
        // the threads table to be that of the most recent message in
        // the thread.
        db.execSQL("  UPDATE threads" + "  SET" + "  date =" + "    (SELECT date FROM"
                + "        (SELECT date * 1000 AS date, thread_id FROM pdu"
                + "         UNION SELECT date, thread_id FROM sms)" + "     WHERE thread_id = " + thread_id
                + " ORDER BY date DESC LIMIT 1)," + "  snippet =" + "    (SELECT snippet FROM"
                + "        (SELECT date * 1000 AS date, sub AS snippet, thread_id FROM pdu"
                + "         UNION SELECT date, body AS snippet, thread_id FROM sms)"
                + "     WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1)," + "  snippet_cs ="
                + "    (SELECT snippet_cs FROM"
                + "        (SELECT date * 1000 AS date, sub_cs AS snippet_cs, thread_id FROM pdu"
                + "         UNION SELECT date, 0 AS snippet_cs, thread_id FROM sms)"
        ...
        String query = "SELECT thread_id FROM sms WHERE type="
                + Telephony.TextBasedSmsColumns.MESSAGE_TYPE_FAILED + " AND thread_id = " + thread_id
                + " LIMIT 1";
        int setError = 0;
    ...
    public static void updateAllThreads(SQLiteDatabase db, String where, String[] whereArgs) {
        ...
        String query = "SELECT _id FROM threads WHERE _id IN " + "(SELECT DISTINCT thread_id FROM sms "
                + where + ")";
        ...
        db.delete("threads", "_id NOT IN (SELECT DISTINCT thread_id FROM sms "
    ...
    public void onCreate(SQLiteDatabase db) {
        if(Logger.IS_DEBUG_ENABLED) {
        	Logger.debug("Creating db: " + db.getPath());
        }
    	createMmsTables(db);
        createSmsTables(db);
    ...
    private void populateWordsTable(SQLiteDatabase db) {
        ...
        {
            Cursor smsRows = db
                    .query("sms", new String[] { Sms._ID, Sms.BODY }, null, null, null, null, null);
            try {
                if (smsRows != null) {
        ...
        {
            Cursor mmsRows = db.query("part", new String[] { Part._ID, Part.TEXT }, "ct = 'text/plain'",
                    null, null, null, null);
            try {
                if (mmsRows != null) {
    ...
    private void createWordsTables(SQLiteDatabase db) {
        try {
            ...
            db.execSQL("CREATE TRIGGER mms_words_update AFTER UPDATE ON part BEGIN UPDATE words "
                    + " SET index_text = NEW.text WHERE (source_id=NEW._id AND table_to_use=2); " + " END;");
        	...
        	Logger.error(getClass(), "got exception creating words table: " + ex.toString());
        }
    }
    ...
    private void createThreadIdIndex(SQLiteDatabase db) {
        try {
        	...
        	Logger.error(getClass(), "got exception creating indices: " + ex.toString());
        }
    }
    ...
    private void createMmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PDU + " (" + Mms._ID + " INTEGER PRIMARY KEY,"
                + Mms.THREAD_ID + " INTEGER," + Mms.DATE + " INTEGER," + Mms.MESSAGE_BOX + " INTEGER,"
                + Mms.READ + " INTEGER DEFAULT 0," + Mms.MESSAGE_ID + " TEXT," + Mms.SUBJECT + " TEXT,"
                + Mms.SUBJECT_CHARSET + " INTEGER," + Mms.CONTENT_TYPE + " TEXT," + Mms.CONTENT_LOCATION
                + " TEXT," + Mms.EXPIRY + " INTEGER," + Mms.MESSAGE_CLASS + " TEXT," + Mms.MESSAGE_TYPE
                + " INTEGER," + Mms.MMS_VERSION + " INTEGER," + Mms.MESSAGE_SIZE + " INTEGER," + Mms.PRIORITY
                ...
                + Mms.RETRIEVE_TEXT_CHARSET + " INTEGER," + Mms.READ_STATUS + " INTEGER," + Mms.CONTENT_CLASS
                + " INTEGER," + Mms.RESPONSE_TEXT + " TEXT," + Mms.DELIVERY_TIME + " INTEGER,"
                + Mms.DELIVERY_REPORT + " INTEGER," + Mms.LOCKED + " INTEGER DEFAULT 0," + Mms.SEEN
                + " INTEGER DEFAULT 0" + ");");

        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_ADDR + " (" + Addr._ID + " INTEGER PRIMARY KEY,"
                + Addr.MSG_ID + " INTEGER," + Addr.CONTACT_ID + " INTEGER," + Addr.ADDRESS + " TEXT,"
                + Addr.TYPE + " INTEGER," + Addr.CHARSET + " INTEGER);");

        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PART + " (" + Part._ID + " INTEGER PRIMARY KEY,"
                + Part.MSG_ID + " INTEGER," + Part.SEQ + " INTEGER DEFAULT 0," + Part.CONTENT_TYPE + " TEXT,"
                + Part.NAME + " TEXT," + Part.CHARSET + " INTEGER," + Part.CONTENT_DISPOSITION + " TEXT,"
                + Part.FILENAME + " TEXT," + Part.CONTENT_ID + " TEXT," + Part.CONTENT_LOCATION + " TEXT,"
                + Part.CT_START + " INTEGER," + Part.CT_TYPE + " TEXT," + Part._DATA + " TEXT," + Part.TEXT
                + " TEXT);");

        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_RATE + " (" + Rate.SENT_TIME + " INTEGER);");

        db.execSQL("CREATE TABLE " + MmsProvider.TABLE_DRM + " (" + BaseColumns._ID + " INTEGER PRIMARY KEY,"
                + "_data TEXT);");
    }

    private void createMmsTriggers(SQLiteDatabase db) {
        // Cleans up parts when a MM is deleted.
        db.execSQL("CREATE TRIGGER part_cleanup DELETE ON " + MmsProvider.TABLE_PDU + " " + "BEGIN "
                + "  DELETE FROM " + MmsProvider.TABLE_PART + "  WHERE " + Part.MSG_ID + "=old._id;" + "END;");

        // Cleans up address info when a MM is deleted.
        db.execSQL("CREATE TRIGGER addr_cleanup DELETE ON " + MmsProvider.TABLE_PDU + " " + "BEGIN "
                + "  DELETE FROM " + MmsProvider.TABLE_ADDR + "  WHERE " + Addr.MSG_ID + "=old._id;" + "END;");
        ...
        db.execSQL("CREATE TRIGGER cleanup_delivery_and_read_report " + "AFTER DELETE ON "
                ...
                + PduHeaders.MESSAGE_TYPE_READ_ORIG_IND + ")" + "    AND " + Mms.MESSAGE_ID + "=old."
                + Mms.MESSAGE_ID + "; " + "END;");
    ...
    private void createSmsTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE sms ("
                ...
                + "service_center TEXT," + "locked INTEGER DEFAULT 0," + "error_code INTEGER DEFAULT 0,"
                + "seen INTEGER DEFAULT 0" + ");");

        /**
         * This table is used by the SMS dispatcher to hold incomplete partial messages until all the parts
         * arrive.
         */
        db.execSQL("CREATE TABLE raw (" + "_id INTEGER PRIMARY KEY," + "date INTEGER,"
                ...
                "destination_port INTEGER," + "address TEXT," + "pdu TEXT);"); // the raw PDU for this part
    ...
    private void createCommonTables(SQLiteDatabase db) {
        ...
        /**
         * This table maps the first instance seen of any particular MMS/SMS address to an ID, which is then
         * used as its canonical representation. If the same address or an equivalent address (as determined
         * by our Sqlite PHONE_NUMBERS_EQUAL extension) is seen later, this same ID will be used.
         */
        db.execSQL("CREATE TABLE canonical_addresses (" + "_id INTEGER PRIMARY KEY," + "address TEXT);");

        /**
         * This table maps the subject and an ordered set of recipient IDs, separated by spaces, to a unique
         * thread ID. The IDs come from the canonical_addresses table. This works because messages are
         * considered to be part of the same thread if they have the same subject (or a null subject) and the
        ...
        db.execSQL("CREATE TABLE threads (" + Threads._ID + " INTEGER PRIMARY KEY," + Threads.DATE
                ...
                + Threads.RECIPIENT_IDS + " TEXT," + Threads.SNIPPET + " TEXT," + Threads.SNIPPET_CHARSET
                + " INTEGER DEFAULT 0," + Threads.READ + " INTEGER DEFAULT 1," + Threads.TYPE
                + " INTEGER DEFAULT 0," + Threads.ERROR + " INTEGER DEFAULT 0," + Threads.HAS_ATTACHMENT
                + " INTEGER DEFAULT 0);");
        ...
        db.execSQL("CREATE TABLE " + MmsSmsProvider.TABLE_PENDING_MSG + " (" + PendingMessages._ID
                + " INTEGER PRIMARY KEY," + PendingMessages.PROTO_TYPE + " INTEGER," + PendingMessages.MSG_ID
                + " INTEGER," + PendingMessages.MSG_TYPE + " INTEGER," + PendingMessages.ERROR_TYPE
                + " INTEGER," + PendingMessages.ERROR_CODE + " INTEGER," + PendingMessages.RETRY_INDEX
                + " INTEGER NOT NULL DEFAULT 0," + PendingMessages.DUE_TIME + " INTEGER,"
...
//        db.execSQL("CREATE TRIGGER pdu_update_thread_on_insert AFTER INSERT ON " + MmsProvider.TABLE_PDU
//                + " " + PDU_UPDATE_THREAD_CONSTRAINTS + PDU_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);
//
...
//                + SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);
        ...
        db.execSQL("CREATE TRIGGER pdu_update_thread_date_subject_on_update AFTER" + "  UPDATE OF "
                + Mms.DATE + ", " + Mms.SUBJECT + ", " + Mms.MESSAGE_BOX + "  ON " + MmsProvider.TABLE_PDU
                + " " + PDU_UPDATE_THREAD_CONSTRAINTS + PDU_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);
        ...
        // Updates threads table whenever a message in pdu is updated.
        db.execSQL("CREATE TRIGGER pdu_update_thread_read_on_update AFTER" + "  UPDATE OF " + Mms.READ
                + "  ON " + MmsProvider.TABLE_PDU + " " + PDU_UPDATE_THREAD_CONSTRAINTS + "BEGIN "
                + PDU_UPDATE_THREAD_READ_BODY + "END;");

        // Updates threads table whenever a message in sms is updated.
        db.execSQL("CREATE TRIGGER sms_update_thread_read_on_update AFTER" + "  UPDATE OF " + Sms.READ
                + "  ON sms " + "BEGIN " + SMS_UPDATE_THREAD_READ_BODY + "END;");

        // Update threads table whenever a message in pdu is deleted
        db.execSQL("CREATE TRIGGER pdu_update_thread_on_delete " + "AFTER DELETE ON pdu " + "BEGIN "
        ...
        db.execSQL("CREATE TRIGGER delete_obsolete_threads_pdu " + "AFTER DELETE ON pdu " + "BEGIN "
                + "  DELETE FROM threads " + "  WHERE " + "    _id = old.thread_id " + "    AND _id NOT IN "
                + "    (SELECT thread_id FROM sms " + "     UNION SELECT thread_id from pdu); " + "END;");

        db.execSQL("CREATE TRIGGER delete_obsolete_threads_when_update_pdu " + "AFTER UPDATE OF "
                + Mms.THREAD_ID + " ON pdu " + "WHEN old." + Mms.THREAD_ID + " != new." + Mms.THREAD_ID + " "
        ...
        db.execSQL("CREATE TRIGGER insert_mms_pending_on_insert " + "AFTER INSERT ON pdu " + "WHEN new."
                + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND + "  OR new."
                + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_READ_REC_IND + " " + "BEGIN "
                + "  INSERT INTO " + MmsSmsProvider.TABLE_PENDING_MSG + "    (" + PendingMessages.PROTO_TYPE
                + "," + "     " + PendingMessages.MSG_ID + "," + "     " + PendingMessages.MSG_TYPE + ","
                + "     " + PendingMessages.ERROR_TYPE + "," + "     " + PendingMessages.ERROR_CODE + ","
                + "     " + PendingMessages.RETRY_INDEX + "," + "     " + PendingMessages.DUE_TIME + ") "
                + "  VALUES " + "    (" + MmsSms.MMS_PROTO + "," + "      new." + BaseColumns._ID + ","
                + "      new." + Mms.MESSAGE_TYPE + ",0,0,0,0);" + "END;");

        // Insert pending status for M-Send.req when it is moved into Outbox.
        db.execSQL("CREATE TRIGGER insert_mms_pending_on_update " + "AFTER UPDATE ON pdu " + "WHEN new."
                + Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_SEND_REQ + "  AND new." + Mms.MESSAGE_BOX
                + "=" + Mms.MESSAGE_BOX_OUTBOX + "  AND old." + Mms.MESSAGE_BOX + "!="
                + Mms.MESSAGE_BOX_OUTBOX + " " + "BEGIN " + "  INSERT INTO "
                + MmsSmsProvider.TABLE_PENDING_MSG + "    (" + PendingMessages.PROTO_TYPE + "," + "     "
                + PendingMessages.MSG_ID + "," + "     " + PendingMessages.MSG_TYPE + "," + "     "
                + PendingMessages.ERROR_TYPE + "," + "     " + PendingMessages.ERROR_CODE + "," + "     "
                + PendingMessages.RETRY_INDEX + "," + "     " + PendingMessages.DUE_TIME + ") " + "  VALUES "
                + "    (" + MmsSms.MMS_PROTO + "," + "      new." + BaseColumns._ID + "," + "      new."
                + Mms.MESSAGE_TYPE + ",0,0,0,0);" + "END;");
        ...
        db.execSQL("CREATE TRIGGER delete_mms_pending_on_update " + "AFTER UPDATE ON "
                + MmsProvider.TABLE_PDU + " " + "WHEN old." + Mms.MESSAGE_BOX + "=" + Mms.MESSAGE_BOX_OUTBOX
                + "  AND new." + Mms.MESSAGE_BOX + "!=" + Mms.MESSAGE_BOX_OUTBOX + " " + "BEGIN "
                + "  DELETE FROM " + MmsSmsProvider.TABLE_PENDING_MSG + "  WHERE " + PendingMessages.MSG_ID
                + "=new._id; " + "END;");
        ...
        db.execSQL("CREATE TRIGGER delete_mms_pending_on_delete " + "AFTER DELETE ON "
                ...
                + MmsSmsProvider.TABLE_PENDING_MSG + "  WHERE " + PendingMessages.MSG_ID + "=old._id; "
                + "END;");
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_update_mms "
                ...
                + "  WHERE _id =" + "   (SELECT DISTINCT thread_id" + "    FROM pdu"
        ...
        db.execSQL("CREATE TRIGGER update_threads_error_on_delete_mms " + "  BEFORE DELETE ON pdu"
                ...
                + "  UPDATE threads SET error = error - 1" + "  WHERE _id = OLD.thread_id; " + "END;");

        // Update the error flag of threads while moving an MM out of Outbox,
        // which was failed to be sent permanently.
        db.execSQL("CREATE TRIGGER update_threads_error_on_move_mms " + "  BEFORE UPDATE OF msg_box ON pdu "
                + "  WHEN (OLD.msg_box = 4 AND NEW.msg_box != 4) "
                + "  AND (OLD._id IN (SELECT DISTINCT msg_id" + "                   FROM pending_msgs"
                + "                   WHERE err_type >= 10)) " + "BEGIN "
                + "  UPDATE threads SET error = error - 1" + "  WHERE _id = OLD.thread_id; " + "END;");

        // Update the error flag of threads after a text message was
        // failed to send/receive.
        db.execSQL("CREATE TRIGGER update_threads_error_on_update_sms " + "  AFTER UPDATE OF type ON sms"
    ...
    protected void postError(SQLiteDatabase db, int oldVersion, int currentVersion) {
    	Map<Thread, StackTraceElement[]> stacks = Thread.getAllStackTraces();
    	StringBuffer buf = new StringBuffer();
    	buf.append(db.getPath() + " DB change request from old=" + oldVersion + " cur=" + currentVersion + "\n");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        if(currentVersion >  oldVersion){
            if (Logger.IS_INFO_ENABLED) {
                Logger.postErrorToAcra(AppDatabaseHelper.class, "Updating the db " + db.getPath() + " from " + oldVersion  + " to "
                        + currentVersion);
                Logger.warn("No updates reusing the old tables.");
            }
            postError(db,oldVersion,currentVersion);
        }else{
            if (Logger.IS_INFO_ENABLED) {
                Logger.postErrorToAcra(AppDatabaseHelper.class, "Downgrading the db " + db.getPath() + " from " + oldVersion + " to "
                        + currentVersion);
            }
...
//
//        Cursor textRows = db.query("part", new String[] { Part._ID, Part._DATA, Part.TEXT },
//                "ct = 'text/plain' OR ct == 'application/smil'", null, null, null, null);
...
//                            // [Jega]
//                            // This Medthod is never going to called by any devices
//
...
//                	Logger.error(getClass(), "unable to clean up old mms file for " + pathToDelete, ex);
//                }
//            }
//            if (textRows != null) {
//                textRows.close();
...
//            if (Logger.IS_DEBUG_ENABLED) {
//                Logger.debug(getClass(), "[MmsSmsDb] upgradeDatabaseToVersion51: updated " + count
...
//            }
//        } catch (Exception ex) {
//            Logger.error(getClass(), "[MmsSmsDb] upgradeDatabaseToVersion51 caught ", ex);
//        }
//    }
//
...
//                + "  (SELECT DISTINCT pdu.thread_id FROM part " + "   JOIN pdu ON pdu._id=part.mid "
//                + "   WHERE part.ct != 'text/plain' AND part.ct != 'application/smil')");
//    }
