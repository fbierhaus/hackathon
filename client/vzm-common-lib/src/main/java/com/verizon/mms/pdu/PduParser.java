...
public class PduParser {
    ...
    public GenericPdu parse(){
        ...
        if ((PduHeaders.MESSAGE_TYPE_SEND_REQ == messageType) ||
        		(PduHeaders.MESSAGE_TYPE_SEND_REQ_X == messageType) ||
    ...
    private static void log(String text) {
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(PduParser.class, text);
        }
    }
    ...
    protected static void parseContentTypeParams(ByteArrayInputStream pduDataStream,
        ...
        while(0 < lastLen) {
            ...
            switch (param) {
                ...
                case PduPart.P_CHARSET:
                    ...
                    if (((firstValue > TEXT_MIN) && (firstValue < TEXT_MAX)) ||
                        ...
                        try {
                        	...
                        	Logger.error(PduParser.class, Arrays.toString(charsetStr), e);
                ...
                default:
                    if (Logger.IS_DEBUG_ENABLED) {
                    	Logger.debug(PduParser.class, "Not supported Content-Type parameter");
                    }
                if (-1 == skipWapValue(pduDataStream, lastLen)) {
                	Logger.error(PduParser.class, "Corrupt Content-Type");
                } else {
                    lastLen = 0;
                }
                break;
    ...
    protected static byte[] parseContentType(ByteArrayInputStream pduDataStream,
        ...
        if (cur < TEXT_MIN) {
            ...
            if (parameterLen < 0) {
            	Logger.error(PduParser.class, "Corrupt MMS message");
                return (PduContentTypes.contentTypes[0]).getBytes(); //"*/*"
            }
        ...
        while(0 < lastLen) {
            ...
            if (header > TEXT_MAX) {
                ...
                switch (header) {
                    ...
                    default:
                        if (Logger.IS_DEBUG_ENABLED) {
                        	Logger.debug(PduParser.class, "Not supported Part headers: " + header);
                        }
                    if (-1 == skipWapValue(pduDataStream, lastLen)) {
                    	Logger.error(PduParser.class, "Corrupt Part headers");
                        return false;
                    }
                    lastLen = 0;
                    break;
