...
/**
 * The class to provide base facility to access MMS related content, which is stored in a SQLite database and
 * in the file system.
 */
public class MmsProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        case MMS_REPORT_STATUS:
            /*
             * SELECT DISTINCT address, T.delivery_status AS delivery_status, T.read_status AS read_status
             * FROM addr INNER JOIN (SELECT P1._id AS id1, P2._id AS id2, P3._id AS id3, ifnull(P2.st, 0) AS
             * delivery_status, ifnull(P3.read_status, 0) AS read_status FROM pdu P1 INNER JOIN pdu P2 ON
             * P1.m_id = P2.m_id AND P2.m_type = 134 LEFT JOIN pdu P3 ON P1.m_id = P3.m_id AND P3.m_type = 136
             * UNION SELECT P1._id AS id1, P2._id AS id2, P3._id AS id3, ifnull(P2.st, 0) AS delivery_status,
             * ifnull(P3.read_status, 0) AS read_status FROM pdu P1 INNER JOIN pdu P3 ON P1.m_id = P3.m_id AND
             * P3.m_type = 136 LEFT JOIN pdu P2 ON P1.m_id = P2.m_id AND P2.m_type = 134) T ON (msg_id = id2
             * AND type = 151) OR (msg_id = id3 AND type = 137) WHERE T.id1 = ?;
             */
            qb.setTables("addr INNER JOIN (SELECT P1._id AS id1, P2._id"
                    ...
                    + " AND P2.m_type=134) T ON (msg_id=id2 AND type=151)" + " OR (msg_id=id3 AND type=137)");
            qb.appendWhere("T.id1 = " + uri.getLastPathSegment());
            qb.setDistinct(true);
            break;
        case MMS_REPORT_REQUEST:
            /*
             * SELECT address, d_rpt, rr FROM addr join pdu on pdu._id = addr.msg_id WHERE pdu._id = messageId
             * AND addr.type = 151
             */
        ...
        default:
            if(Logger.IS_ERROR_ENABLED){
                Logger.error(getClass(), "query: invalid request: " + uri);
            }
            return null;
        }
...
//		}
    ...
    public String getType(Uri uri) {
        ...
        case MMS_PART_ID: {
            Cursor cursor = mOpenHelper.getReadableDatabase().query(TABLE_PART,
                    new String[] { Part.CONTENT_TYPE }, Part._ID + " = ?",
            ...
            if (cursor != null) {
                try {
                    if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    	...
                    	Logger.error(getClass(), "cursor.count() != 1: " + uri);
                    }
                } finally {
                    cursor.close();
                }
            } else {
            	Logger.error(getClass(), "cursor == null: " + uri);
            }
            return "*/*";
        }
        case MMS_ALL_PART:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        case MMS_DRM_STORAGE:
            ...
            break;
        default:
        	Logger.error(getClass(), "insert: invalid request: " + uri);
            return null;
        }
...
//			}
        ...
        if (table.equals(TABLE_PDU)) {
            ...
            if ((rowId = db.insert(table, null, finalValues)) <= 0) {
                if(Logger.IS_DEBUG_ENABLED){
                    Logger.debug("Updating thread Ids ");
                }
            	Logger.error(getClass(), "MmsProvider.insert: failed! " + finalValues);
                return null;
            }
            if(finalValues.containsKey(Mms.THREAD_ID)){
                MmsSmsDatabaseHelper.updateThread(db, finalValues.getAsLong(Mms.THREAD_ID));
            }
            ...
            if ((rowId = db.insert(table, null, finalValues)) <= 0) {
            	Logger.error(getClass(), "Failed to insert address: " + finalValues);
                return null;
            }
            ...
            if (!plainText && !smilText) {
                ...
                // permission settings.
                String path = getContext().getDir("parts", 0).getPath() + "/PART_"
                ...
                if (!partFile.exists()) {
                    try {
                    	...
                    	Logger.error(getClass(), "createNewFile", e);
                        throw new IllegalStateException("Unable to create new partFile: " + path);
                    }
                }
            }
            ...
            String path = getContext().getDir("parts", 0).getPath() + "/PART_" + System.currentTimeMillis();
		...
		if (Logger.IS_DEBUG_ENABLED) {
			Logger.debug(getClass(), "insert: time = " + (SystemClock.uptimeMillis() - start) +
				"ms, uri = " + uri + ", values = " + values + ", returning " + res);
		}
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        case MMS_DRM_STORAGE:
            ...
            break;
        default:
        	Logger.warn(getClass(), "No match for URI '" + uri + "'");
            return 0;
        }
...
//			}
    ...
    static int deleteMessages(Context context, SQLiteDatabase db, String selection, String[] selectionArgs,
            Uri uri) {
        Cursor cursor = db.query(TABLE_PDU, new String[] { Mms._ID }, selection, selectionArgs, null, null,
                null);
        if (cursor == null) {
            return 0;
        }
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                deleteParts(db, Part.MSG_ID + " = ?", new String[] { String.valueOf(cursor.getLong(0)) });
            }
        } finally {
            cursor.close();
        }
        ...
        if (count > 0) {
            ...
            if(Logger.IS_DEBUG_ENABLED){
            	Logger.debug(MmsProvider.class, "Broadcasting intent: " + intent);
            }
    ...
    private static int deleteDataRows(SQLiteDatabase db, String table, String selection,
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                try {
                	...
                	Logger.error(MmsProvider.class, ex.getMessage(), ex);
                }
            }
        } finally {
            cursor.close();
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        case MMS_PART_ID:
            ...
            break;
        default:
        	Logger.warn(getClass(), "Update operation for '" + uri + "' not implemented.");
            return 0;
        }
...
//			}
    ...
    private ParcelFileDescriptor getTempStoreFd() {
        ...
        try {
            ...
            if (!parentFile.exists() && !parentFile.mkdirs()) {
            	Logger.error(getClass(), "[MmsProvider] getTempStoreFd: " + parentFile.getPath() + "does not exist!");
                return null;
            }
        	...
        	Logger.error(getClass(), "getTempStoreFd: error creating pfd for " + fileName, ex);
        }
    ...
    public ParcelFileDescriptor openFile(Uri uri, String mode) throws FileNotFoundException {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
        	Logger.debug(getClass(), "openFile: uri=" + uri + ", mode=" + mode);
        }
    ...
    static {
        sURLMatcher.addURI(AUTHORITY, null, MMS_ALL);
        sURLMatcher.addURI(AUTHORITY, "#", MMS_ALL_ID);
        sURLMatcher.addURI(AUTHORITY, "inbox", MMS_INBOX);
        sURLMatcher.addURI(AUTHORITY, "inbox/#", MMS_INBOX_ID);
        sURLMatcher.addURI(AUTHORITY, "sent", MMS_SENT);
        sURLMatcher.addURI(AUTHORITY, "sent/#", MMS_SENT_ID);
        sURLMatcher.addURI(AUTHORITY, "drafts", MMS_DRAFTS);
        sURLMatcher.addURI(AUTHORITY, "drafts/#", MMS_DRAFTS_ID);
        sURLMatcher.addURI(AUTHORITY, "outbox", MMS_OUTBOX);
        sURLMatcher.addURI(AUTHORITY, "outbox/#", MMS_OUTBOX_ID);
        sURLMatcher.addURI(AUTHORITY, "part", MMS_ALL_PART);
        sURLMatcher.addURI(AUTHORITY, "#/part", MMS_MSG_PART);
        sURLMatcher.addURI(AUTHORITY, "part/#", MMS_PART_ID);
        sURLMatcher.addURI(AUTHORITY, "#/addr", MMS_MSG_ADDR);
        sURLMatcher.addURI(AUTHORITY, "rate", MMS_SENDING_RATE);
        sURLMatcher.addURI(AUTHORITY, "report-status/#", MMS_REPORT_STATUS);
        sURLMatcher.addURI(AUTHORITY, "report-request/#", MMS_REPORT_REQUEST);
        sURLMatcher.addURI(AUTHORITY, "drm", MMS_DRM_STORAGE);
        sURLMatcher.addURI(AUTHORITY, "drm/#", MMS_DRM_STORAGE_ID);
        sURLMatcher.addURI(AUTHORITY, "threads", MMS_THREADS);
