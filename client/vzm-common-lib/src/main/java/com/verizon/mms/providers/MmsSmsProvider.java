...
/**
 * This class provides the ability to query the MMS and SMS databases at the same time, mixing messages from
 * both in a single thread (A.K.A. conversation).
 * 
 * A virtual column, MmsSms.TYPE_DISCRIMINATOR_COLUMN, may be requested in the projection for a query. Its
 * value is either "mms" or "sms", depending on whether the message represented by the row is an MMS message
 * or an SMS message, respectively.
 * 
 * This class also provides the ability to find out what addresses participated in a particular thread. It
 * doesn't support updates for either of these.
 * 
 * This class provides a way to allocate and retrieve thread IDs. This is done atomically through a query.
 * There is no insert URI for this.
 * 
 * Finally, this class provides a way to delete or update all messages in a thread.
 */
public class MmsSmsProvider extends ContentProvider {
    ...
    /**
     * the name of the table that is used to store the queue of messages(both MMS and SMS) to be
     * sent/downloaded.
     */
    ...
    private static final String[] MMS_SMS_COLUMNS = { BaseColumns._ID, Mms.DATE, Mms.READ, Mms.THREAD_ID,
            Mms.LOCKED };

    // These are the columns that appear only in the MMS message
    // table.
    private static final String[] MMS_ONLY_COLUMNS = { Mms.CONTENT_CLASS, Mms.CONTENT_LOCATION,
            Mms.CONTENT_TYPE, Mms.DELIVERY_REPORT, Mms.EXPIRY, Mms.MESSAGE_CLASS, Mms.MESSAGE_ID,
            Mms.MESSAGE_SIZE, Mms.MESSAGE_TYPE, Mms.MESSAGE_BOX, Mms.PRIORITY, Mms.READ_STATUS,
            Mms.RESPONSE_STATUS, Mms.RESPONSE_TEXT, Mms.RETRIEVE_STATUS, Mms.RETRIEVE_TEXT_CHARSET,
            Mms.REPORT_ALLOWED, Mms.READ_REPORT, Mms.STATUS, Mms.SUBJECT, Mms.SUBJECT_CHARSET,
            Mms.TRANSACTION_ID, Mms.MMS_VERSION };

    // These are the columns that appear only in the SMS message
    // table.
    private static final String[] SMS_ONLY_COLUMNS = { "address", "body", "person", "reply_path_present",
            "service_center", "status", "subject", "type", "error_code" };

    // These are all the columns that appear in the "threads" table.
    private static final String[] THREADS_COLUMNS = { BaseColumns._ID, ThreadsColumns.DATE,
            ThreadsColumns.RECIPIENT_IDS, ThreadsColumns.MESSAGE_COUNT };
    ...
    // message tables.
    private static final String[] UNION_COLUMNS = new String[MMS_SMS_COLUMNS.length + MMS_ONLY_COLUMNS.length
    ...
    private static final String MMS_CONVERSATION_CONSTRAINT = "(" + Mms.MESSAGE_BOX + " != "
            + Mms.MESSAGE_BOX_DRAFTS + " AND (" + Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_SEND_REQ
            + " OR " + Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF + " OR "
...
//		}
        ...
        case URI_COMPLETE_CONVERSATIONS:
            cursor = getCompleteConversations(projection, selection, selectionArgs, sortOrder);
            break;
        case URI_CONVERSATIONS:
            ...
            if ((simple != null) && simple.equals("true")) {
                ...
                cursor = getConversations(projection, selection, selectionArgs, sortOrder);
            }
            break;
        case URI_CONVERSATIONS_MESSAGES:
            cursor = getConversationMessages(uri.getPathSegments().get(1), projection, selection,
                    selectionArgs, sortOrder);
            break;
        case URI_CONVERSATIONS_RECIPIENTS:
            cursor = getConversationById(uri.getPathSegments().get(1), projection, selection, selectionArgs,
                    sortOrder);
            break;
        ...
        case URI_MESSAGES_BY_PHONE:
            cursor = getMessagesByPhoneNumber(uri.getPathSegments().get(2), projection, selection,
                    selectionArgs, sortOrder);
            break;
        case URI_THREAD_ID:
        ...
        case URI_CANONICAL_ADDRESS: {
            ...
            String finalSelection = TextUtils.isEmpty(selection) ? extraSelection : extraSelection + " AND "
                    + selection;
            cursor = db.query(TABLE_CANONICAL_ADDRESSES, CANONICAL_ADDRESSES_COLUMNS_1, finalSelection,
                    selectionArgs, null, null, sortOrder);
            break;
        }
        case URI_CANONICAL_ADDRESSES:
            cursor = db.query(TABLE_CANONICAL_ADDRESSES, CANONICAL_ADDRESSES_COLUMNS_2, selection,
                    selectionArgs, null, null, sortOrder);
            break;
        case URI_SEARCH_SUGGEST: {
            String searchString = uri.getQueryParameter("pattern");
            String query = String
                    .format("SELECT _id, index_text, source_id, table_to_use, offsets(words) FROM words WHERE words MATCH '%s*' LIMIT 50;",
                            searchString);
        ...
        case URI_SEARCH: {
            ...
            String smsQuery = String.format("SELECT %s FROM sms,words WHERE (words MATCH ? "
                    + " AND sms._id=words.source_id AND words.table_to_use=1) ", smsProjection);
            ...
            String mmsQuery = String.format(
                    "SELECT %s FROM pdu,part,addr,words WHERE ((part.mid=pdu._id) AND "
                            ...
                            + "(part.ct='text/plain') AND " + "(words MATCH ?) AND "
                            + "(part._id = words.source_id) AND " + "(words.table_to_use=2))", mmsProjection,
                    PduHeaders.TO);

            // join the results from sms and part (mms)
            String rawQuery = String.format("%s UNION %s GROUP BY %s ORDER BY %s", smsQuery, mmsQuery,
                    "thread_id", "thread_id ASC, date DESC");
            try {
                cursor = db.rawQuery(rawQuery, new String[] { searchString, searchString });
            } catch (Exception ex) {
                if(Logger.IS_DEBUG_ENABLED){
                    Logger.error(getClass(), "got exception: " + ex.toString());
                }
            }
            break;
        }
        case URI_PENDING_MSG: {
            ...
            int proto = TextUtils.isEmpty(protoName) ? -1 : (protoName.equals("sms") ? MmsSms.SMS_PROTO
                    : MmsSms.MMS_PROTO);

            String extraSelection = (proto != -1) ? (PendingMessages.PROTO_TYPE + "=" + proto) : " 0=0 ";
            if (!TextUtils.isEmpty(msgId)) {
                extraSelection += " AND " + PendingMessages.MSG_ID + "=" + msgId;
            }

            String finalSelection = TextUtils.isEmpty(selection) ? extraSelection : ("(" + extraSelection
                    + ") AND " + selection);
            String finalOrder = TextUtils.isEmpty(sortOrder) ? PendingMessages.DUE_TIME : sortOrder;
        ...
        case URI_FIRST_LOCKED_MESSAGE_BY_THREAD_ID: {
            ...
            cursor = getFirstLockedMessage(projection, "thread_id=" + Long.toString(threadId), null,
                    sortOrder);
            break;
        }
        case URI_FIRST_LOCKED_MESSAGE_ALL: {
            cursor = getFirstLockedMessage(projection, selection, selectionArgs, sortOrder);
            break;
        }
        default:
...
//				"ms, uri = " + uri + ", cols = " + Arrays.toString(projection) +
//				", selection = <" + selection + ">, args = " + Arrays.toString(selectionArgs) +
//				", returning " + (cursor == null ? "null" : cursor.getCount()));
//		}
    ...
    private long getSingleAddressId(String address) {
        ...
        try {
            SQLiteDatabase db = mOpenHelper.getReadableDatabase();
            cursor = db.query("canonical_addresses", ID_PROJECTION, selection, selectionArgs, null, null,
                    null);
...
//                if (Logger.IS_DEBUG_ENABLED) {
//                    Logger.debug(getClass(), "getSingleAddressId: insert new canonical_address for "
//                            + address + ", _id=" + retVal);
//                }
                return retVal;
            }
        ...
        for (String address : addresses) {
            if (!address.equals(PduHeaders.FROM_INSERT_ADDRESS_TOKEN_STR)) {
                ...
                if (id != -1L) {
                    ...
                    Logger.error(MmsSmsProvider.class, "getAddressIds: address ID not found for " + address);
                }
            }
        }
        return result;
    ...
    private void insertThread(String recipientIds, int numberOfRecipients) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(getClass(), "insertThread: created new thread_id " + result + " for recipientIds "
                    + recipientIds);
        }
    ...
    /**
     * Return the thread ID for this list of recipients IDs. If no thread exists with this ID, create one and
     * return it. Callers should always use VZTelephony.getThreadId to access this information.
     */
    private synchronized Cursor getThreadId(List<String> recipients) {
        ...
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(getClass(), "getThreadId: recipientIds (selectionArgs) =" + recipientIds);
        }
        ...
        if (cursor.getCount() == 0) {
            cursor.close();
            if (Logger.IS_DEBUG_ENABLED) {
                Logger.debug(getClass(), "getThreadId: create new thread_id for recipients " + recipients);
            }
        ...
        if (cursor.getCount() > 1) {
            if (Logger.IS_DEBUG_ENABLED) {
                Logger.debug(getClass(), "getThreadId: why is cursorCount=" + cursor.getCount());
            }
        }
    ...
    /**
     * If a null projection is given, return the union of all columns in both the MMS and SMS messages tables.
     * Otherwise, return the given projection.
     */
    ...
    /**
     * If a null projection is given, return the set of all columns in the threads table. Otherwise, return
     ...
     */
    private Cursor getSimpleConversations(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        return mOpenHelper.getReadableDatabase().query("threads", projection, selection, selectionArgs, null,
                null, " date DESC");
    }
     ...
     * Use this query:
     * 
     * SELECT ... FROM (SELECT _id, thread_id, ... FROM pdu WHERE msg_box = 3 AND ... UNION SELECT _id,
     * thread_id, ... FROM sms WHERE type = 3 AND ... ) ;
     */
    private Cursor getDraftThread(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        ...
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerProjection, MMS_COLUMNS, 1, "mms",
                concatSelections(selection, Mms.MESSAGE_BOX + "=" + Mms.MESSAGE_BOX_DRAFTS), selectionArgs,
                null, null);
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerProjection, SMS_COLUMNS, 1, "sms",
                concatSelections(selection, Sms.TYPE + "=" + Sms.MESSAGE_TYPE_DRAFT), selectionArgs, null,
                null);
        SQLiteQueryBuilder unionQueryBuilder = new SQLiteQueryBuilder();

        unionQueryBuilder.setDistinct(true);

        String unionQuery = unionQueryBuilder.buildUnionQuery(new String[] { mmsSubQuery, smsSubQuery },
                null, null);
     ...
     * Use this query:
     * 
     * SELECT ... FROM (SELECT thread_id AS tid, date * 1000 AS normalized_date, ... FROM pdu WHERE msg_box !=
     * 3 AND ... GROUP BY thread_id HAVING date = MAX(date) UNION SELECT thread_id AS tid, date AS
     * normalized_date, ... FROM sms WHERE ... GROUP BY thread_id HAVING date = MAX(date)) GROUP BY tid HAVING
     * normalized_date = MAX(normalized_date);
     * 
     * The msg_box != 3 comparisons ensure that we don't include draft messages.
     */
    private Cursor getConversations(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        SQLiteQueryBuilder mmsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
        ...
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerMmsProjection, MMS_COLUMNS, 1, "mms",
                concatSelections(selection, MMS_CONVERSATION_CONSTRAINT), selectionArgs, "thread_id",
                "date = MAX(date)");
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerSmsProjection, SMS_COLUMNS, 1, "sms",
                concatSelections(selection, SMS_CONVERSATION_CONSTRAINT), selectionArgs, "thread_id",
                "date = MAX(date)");
        SQLiteQueryBuilder unionQueryBuilder = new SQLiteQueryBuilder();
        ...
        String outerQuery = outerQueryBuilder.buildQuery(columns, null, null, "tid",
    ...
    /**
     * Return the first locked message found in the union of MMS and SMS messages.
     * 
     * Use this query:
     * 
     * SELECT _id FROM pdu GROUP BY _id HAVING locked=1 UNION SELECT _id FROM sms GROUP BY _id HAVING locked=1
     * LIMIT 1
     * 
     * We limit by 1 because we're only interested in knowing if there is *any* locked message, not the actual
     * messages themselves.
     */
    private Cursor getFirstLockedMessage(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        SQLiteQueryBuilder mmsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
        ...
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN, idColumn,
                null, 1, "mms", selection, selectionArgs, BaseColumns._ID, "locked=1");

        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN, idColumn,
                null, 1, "sms", selection, selectionArgs, BaseColumns._ID, "locked=1");
        ...
        if (Logger.IS_DEBUG_ENABLED) {
            Logger.debug(getClass(), "MmsSmsProvider", "getFirstLockedMessage query: " + unionQuery);
            Logger.debug(getClass(), "MmsSmsProvider", "cursor count: " + cursor.getCount());
        }
        return cursor;
    }
     ...
     */
    private Cursor getCompleteConversations(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        String unionQuery = buildConversationQuery(projection, selection, selectionArgs, sortOrder);

        return mOpenHelper.getReadableDatabase().rawQuery(unionQuery, EMPTY_STRING_ARRAY);
    }

    /**
     * Add normalized date and thread_id to the list of columns for an inner projection. This is necessary so
     * that the outer query can have access to these columns even if the caller hasn't requested them in the
     * result.
     */
     ...
     */
    private Cursor getConversationMessages(String threadIdString, String[] projection, String selection,
        ...
        String unionQuery = buildConversationQuery(projection, finalSelection, selectionArgs, sortOrder);

        return mOpenHelper.getReadableDatabase().rawQuery(unionQuery, EMPTY_STRING_ARRAY);
    }

    /**
     * Return the union of MMS and SMS messages whose recipients included this phone number.
     * 
     * Use this query:
     * 
     * SELECT ... FROM pdu, (SELECT _id AS address_id FROM addr WHERE (address='<phoneNumber>' OR
     * PHONE_NUMBERS_EQUAL(addr.address, '<phoneNumber>', 1/0))) AS matching_addresses WHERE pdu._id =
     * matching_addresses.address_id UNION SELECT ... FROM sms WHERE (address='<phoneNumber>' OR
     * PHONE_NUMBERS_EQUAL(sms.address, '<phoneNumber>', 1/0));
     */
    private Cursor getMessagesByPhoneNumber(String phoneNumber, String[] projection, String selection,
            String[] selectionArgs, String sortOrder) {
        String escapedPhoneNumber = DatabaseUtils.sqlEscapeString(phoneNumber);
        String finalMmsSelection = concatSelections(selection, "pdu._id = matching_addresses.address_id");
        String finalSmsSelection = concatSelections(selection, "(address=" + escapedPhoneNumber
                + " OR PHONE_NUMBERS_EQUAL(address, " + escapedPhoneNumber
                + (mUseStrictPhoneNumberComparation ? ", 1))" : ", 0))"));
        SQLiteQueryBuilder mmsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
        ...
        mmsQueryBuilder.setTables(MmsProvider.TABLE_PDU + ", (SELECT _id AS address_id "
                + "FROM addr WHERE (address=" + escapedPhoneNumber + " OR PHONE_NUMBERS_EQUAL(addr.address, "
                + escapedPhoneNumber + (mUseStrictPhoneNumberComparation ? ", 1))) " : ", 0))) ")
                + "AS matching_addresses");
        smsQueryBuilder.setTables(SmsProvider.TABLE_SMS);

        String[] columns = handleNullMessageProjection(projection);
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN, columns,
                MMS_COLUMNS, 0, "mms", finalMmsSelection, selectionArgs, null, null);
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN, columns,
                SMS_COLUMNS, 0, "sms", finalSmsSelection, selectionArgs, null, null);
        SQLiteQueryBuilder unionQueryBuilder = new SQLiteQueryBuilder();
     ...
     */
    private Cursor getConversationById(String threadIdString, String[] projection, String selection,
            String[] selectionArgs, String sortOrder) {
        try {
            Long.parseLong(threadIdString);
        } catch (NumberFormatException exception) {
        ...
        return queryBuilder.query(mOpenHelper.getReadableDatabase(), columns, finalSelection, selectionArgs,
                sortOrder, null, null);
    }
    ...
    private Cursor getUndeliveredMessages(String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
        ...
        String finalSmsSelection = concatSelections(selection, "(" + Sms.TYPE + " = "
                + Sms.MESSAGE_TYPE_OUTBOX + " OR " + Sms.TYPE + " = " + Sms.MESSAGE_TYPE_FAILED + " OR "
                + Sms.TYPE + " = " + Sms.MESSAGE_TYPE_QUEUED + ")");
        ...
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerMmsProjection, columnsPresentInTable, 1, "mms", finalMmsSelection, selectionArgs, null,
                null);
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerSmsProjection, SMS_COLUMNS, 1, "sms", finalSmsSelection, selectionArgs, null, null);
        SQLiteQueryBuilder unionQueryBuilder = new SQLiteQueryBuilder();

        unionQueryBuilder.setDistinct(true);

        String unionQuery = unionQueryBuilder.buildUnionQuery(new String[] { smsSubQuery, mmsSubQuery },
                null, null);
    ...
    private static String buildConversationQuery(String[] projection, String selection,
            String[] selectionArgs, String sortOrder) {
        String[] mmsProjection = createMmsProjection(projection);
        ...
        String mmsSelection = concatSelections(selection, Mms.MESSAGE_BOX + " != " + Mms.MESSAGE_BOX_DRAFTS);
        String mmsSubQuery = mmsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerMmsProjection, columnsPresentInTable, 0, "mms",
                concatSelections(mmsSelection, MMS_CONVERSATION_CONSTRAINT), selectionArgs, null, null);
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(MmsSms.TYPE_DISCRIMINATOR_COLUMN,
                innerSmsProjection, SMS_COLUMNS, 0, "sms",
                concatSelections(selection, SMS_CONVERSATION_CONSTRAINT), selectionArgs, null, null);
        SQLiteQueryBuilder unionQueryBuilder = new SQLiteQueryBuilder();
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        case URI_PENDING_MSG:
            affectedRows = db.delete(TABLE_PENDING_MSG, selection, selectionArgs);
            break;
        ...
        case URI_OBSOLETE_THREADS:
            affectedRows = db.delete("threads", "_id NOT IN (SELECT DISTINCT thread_id FROM sms "
...
//				"ms, uri = " + uri + ", where = <" + selection + ">, args = " + Arrays.toString(selectionArgs) +
//				", returning " + affectedRows);
//		}
    ...
    private int deleteConversation(Uri uri, String selection, String[] selectionArgs) {
        ...
        return MmsProvider.deleteMessages(getContext(), db, finalSelection, selectionArgs, uri)
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (URI_MATCHER.match(uri)) {
        case URI_PENDING_MSG:
            // Add the messages in pending table
            SQLiteDatabase db = mOpenHelper.getWritableDatabase();
            long rowId = 0;
            if ((rowId = db.insert(TABLE_PENDING_MSG, null, values)) > 0) {
                return  ContentUris.withAppendedId(VZUris.getMmsSmsPendingUri(), rowId);
            }
            break;
        }
        throw new UnsupportedOperationException(NO_DELETES_INSERTS_OR_UPDATES);
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        case URI_CONVERSATIONS_MESSAGES:
            ...
            affectedRows = updateConversation(threadIdString, values, selection, selectionArgs);
            break;
    ...
    /**
     * Construct Sets of Strings containing exactly the columns present in each table. We will use this when
     * constructing UNION queries across the MMS and SMS tables.
     */
